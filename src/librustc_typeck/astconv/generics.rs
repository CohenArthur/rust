//! The `AstConvGeneric` trait is a subtrait of the `AstConv` trait. The methods present
//! here help when handling generic types from the AST reprsentation.
...
pub trait AstConvGeneric<'tcx>: AstConv<'tcx> {}

impl<'tcx> dyn AstConvGeneric<'tcx> {
    ...
    pub fn prohibit_explicit_late_bound_lifetimes(
        tcx: TyCtxt<'_>,
        def: &ty::Generics,
        args: &hir::GenericArgs<'_>,
        position: GenericArgPosition,
